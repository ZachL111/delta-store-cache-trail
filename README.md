...
delta-store-cache-trail is a Swift project for databases. It focuses on this technical goal: Develop a Swift command-oriented project for cache scenarios with deny and allow fixtures, explainable decision traces, and local-only command execution.
...
The core module exposes a small scoring API. Inputs are simple numeric signals: demand, capacity, latency, risk, and weight. The score uses a threshold of 183, risk penalty 5, latency penalty 4, and weight bonus 5. Tests exercise the public API against the fixture cases in `fixtures/cases.csv`.
